...
impl Runtime {
    ...
    /// compiled. Keep the selected backend while evaluating the management
    /// form so its `require` calls retain the direct-native namespace loader;
    /// only the management form itself re-enters the tree evaluator.
    #[cfg(all(feature = "direct-native", not(target_arch = "wasm32")))]
    fn eval_direct_native_management_form(
        ...
        core::with_exception_site(site, || self.eval_forms(vec![form], false))
    }
...
impl Runtime {
    ...
    pub fn compile_source_foundation_image(
        ...
        let mut owned = Vec::with_capacity(EAGER_HAL_RESOURCES.len() + 1);
        for name in std::iter::once("std.foundation").chain(EAGER_HAL_RESOURCES.iter().copied())
        {
            let Some(path) = catalog.path(name) else {
                if name == "std.foundation" {
                    return Err("Foundation image requires source-owned std.foundation".into());
                }
                continue;
            };
            let source = std::fs::read_to_string(&path)
                .map_err(|error| format!("cannot read Foundation source {}: {error}", path.display()))?;
            owned.push((name.to_owned(), source));
        }
        let modules = owned
            .iter()
    ...
    #[cfg(all(feature = "direct-native", not(target_arch = "wasm32")))]
    pub fn bootstrap_source_foundation_image(&mut self, image: &[u8]) -> Result<(), String> {
        self.configure_execution_backend("direct-native")?;
        crate::vm::eval_bytecode_bundle(self, image)?;
        self.finish_source_foundation_bootstrap();
        Ok(())
    }
...
impl Runtime {
    ...
    /// Installs an already-configured source-program cache for a runtime
    /// owner that shares the same project cache with child sessions.
