...
//! It never runs grammar callbacks or emits code; those responsibilities remain in the owning HAL package.
    ...
    match method {
        ...
        value if value == accessor => {
            ...
            STATE.with(|state| match state.borrow().records.get(&handle) {
                Some(Record::Immutable {
                    type_name: actual,
                    data,
                }) if actual == type_name => Ok(data.clone()),
                _ => Err(failure(
                    type_name,
                    method,
                    "expects its matching std.lang value",
                )),
            })
        }
        _ => Err(failure(type_name, method, "is not installed")),
...
fn library(method: &str, values: Vec<Value>) -> Result<Value, String> {
    match method {
        ...
        "install" => {
            ...
            let key =
                immutable_data(book, "Book", method).and_then(|data| book_key(&data, method))?;
            STATE.with(|state| {
                let mut state = state.borrow_mut();
                let Some(Record::Library {
                    books, revision, ..
                }) = state.records.get_mut(&library)
                else {
                    return Err(failure(
                        "Library",
                        method,
                        "expects a std.lang.Library value",
                    ));
                };
        ...
        "state" => {
            ...
            with_library(library, method, |_, books, revision| {
                Ok(map(vec![
                    ...
                    (
                        "books",
                        Value::Vector(PVector::from_iter(books.values().cloned())),
                    ),
                ]))
            })
        }
...
fn snapshot(method: &str, values: Vec<Value>) -> Result<Value, String> {
    ...
    STATE.with(|state| match state.borrow().records.get(&snapshot) {
        Some(Record::Snapshot {
            ...
            (
                "kind",
                Value::Keyword(Keyword::from(match kind {
                    SnapshotKind::Library => "library",
                    SnapshotKind::Harness => "harness",
                })),
            ),
        ...
        _ => Err(failure(
            "Snapshot",
            method,
            "expects a std.lang.Snapshot value",
        )),
    })
}
...
fn runtime(method: &str, values: Vec<Value>) -> Result<Value, String> {
    match method {
        "create" => {
            ...
            Ok(allocate(
                Record::Runtime {
                    config: config(&values[0], "Runtime/create")?,
                    closed: false,
                    revision: 0,
                },
                "Runtime",
...
fn runtime_mutate(
    ...
    STATE.with(|state| {
        ...
        let Some(Record::Runtime {
            closed, revision, ..
        }) = state.records.get_mut(&runtime)
        else {
            return Err(failure(
                "Runtime",
                method,
                "expects a std.lang.Runtime value",
            ));
        };
        if close {
            *closed = true;
...
fn harness(method: &str, values: Vec<Value>) -> Result<Value, String> {
    match method {
        "create" => {
            ...
            let library =
                optional_handle(&config, "library", "Library", method)?.unwrap_or_else(|| {
                    allocate(
                        Record::Library {
                            config: empty_map(),
                            books: HashMap::new(),
            ...
            let runtime =
                optional_handle(&config, "runtime", "Runtime", method)?.unwrap_or_else(|| {
                    allocate(
                        Record::Runtime {
                            config: empty_map(),
                            closed: false,
        ...
        "library" => {
            require_arity("Harness", method, &values, 1)?;
            let harness = handle(&values[0], "Harness", method)?;
            with_harness(harness, method, |_, library, _, _| {
                Ok(extension("Library", *library))
            })
        }
        "runtime" => {
            require_arity("Harness", method, &values, 1)?;
            let harness = handle(&values[0], "Harness", method)?;
            with_harness(harness, method, |_, _, runtime, _| {
                Ok(extension("Runtime", *runtime))
            })
        }
...
fn harness_snapshot(method: &str, values: Vec<Value>) -> Result<Value, String> {
    require_arity("Harness", method, &values, 1)?;
    let harness = handle(&values[0], "Harness", method)?;
    let (_, library, runtime, closed) =
        with_harness(harness, method, |config, library, runtime, closed| {
            Ok((config.clone(), *library, *runtime, *closed))
        })?;
    let (books, library_revision) = with_library(library, method, |_, books, revision| {
        Ok((books.clone(), *revision))
    })?;
    let (_, runtime_closed, runtime_revision) =
        with_runtime(runtime, method, |config, closed, revision| {
            Ok((config.clone(), *closed, *revision))
        })?;
    Ok(allocate(
        Record::Snapshot {
            kind: SnapshotKind::Harness,
            owner: harness,
            books,
            library_revision,
            runtime_closed,
            runtime_revision,
            harness_closed: closed,
        },
        "Snapshot",
    ))
}
...
fn harness_restore(method: &str, values: Vec<Value>) -> Result<Value, String> {
    ...
    let harness = handle(&values[0], "Harness", method)?;
    let snapshot = handle(&values[1], "Snapshot", method)?;
    let (books, library_revision, runtime_closed, runtime_revision, harness_closed) =
        snapshot_harness(snapshot, harness, method)?;
    let (_, library, runtime, _) =
        with_harness(harness, method, |config, library, runtime, closed| {
            Ok((config.clone(), *library, *runtime, *closed))
        })?;
    ...
    STATE.with(|state| match state.borrow_mut().records.get_mut(&harness) {
        Some(Record::Harness { closed, .. }) => {
            *closed = harness_closed;
            Ok(values[0].clone())
        }
        _ => Err(failure(
            "Harness",
            method,
            "expects a std.lang.Harness value",
        )),
    })
}
...
fn harness_close(method: &str, values: Vec<Value>) -> Result<Value, String> {
    require_arity("Harness", method, &values, 1)?;
    let harness = handle(&values[0], "Harness", method)?;
    let (_, _, runtime, _) = with_harness(harness, method, |config, library, runtime, closed| {
        Ok((config.clone(), *library, *runtime, *closed))
    })?;
...
fn harness_state(method: &str, values: Vec<Value>) -> Result<Value, String> {
    ...
    let library_value = with_library(library, method, |_, books, revision| {
        Ok(map(vec![
            ("revision", Value::Number(*revision)),
            ("book-count", Value::Number(books.len() as i64)),
            (
    ...
    let runtime_value = with_runtime(runtime, method, |_, runtime_closed, revision| {
        Ok(runtime_state_value(*runtime_closed, *revision))
    })?;
    Ok(map(vec![
        (
            "state",
            Value::Keyword(Keyword::from(if closed { "closed" } else { "ready" })),
        ),
...
impl ExtensionHandle for Value {
    fn extension_handle(&self) -> Option<u64> {
        match self {
            Value::Extension(value) if value.provider == PROVIDER => Some(value.handle),
            _ => None,
        }
    }
}
...
fn handle(value: &Value, expected: &str, method: &str) -> Result<u64, String> {
    match value {
        Value::Extension(value) if value.provider == PROVIDER && value.type_name == expected => {
            Ok(value.handle)
        }
        _ => Err(failure(
            expected,
            method,
            &format!("expects a std.lang.{expected} value"),
        )),
    }
}
...
fn config(value: &Value, operation: &str) -> Result<Value, String> {
    map_entries(value)
        .map(|_| value.clone())
        .ok_or_else(|| format!("std.lang.{operation} expects a configuration map"))
}
...
fn book_key_argument(value: &Value, method: &str) -> Result<String, String> {
    if let Value::Extension(extension) = value {
        if extension.provider == PROVIDER && extension.type_name == "Book" {
            return immutable_data(extension.handle, "Book", method)
                .and_then(|data| book_key(&data, method));
        }
    }
    if map_entries(value).is_some() {
        return book_key(value, method);
    }
    Ok(value.display())
}
...
fn with_library<T>(
    handle: u64,
    method: &str,
    operation: impl FnOnce(&Value, &HashMap<String, Value>, &i64) -> Result<T, String>,
) -> Result<T, String> {
    STATE.with(|state| match state.borrow().records.get(&handle) {
        Some(Record::Library {
            config,
            books,
            revision,
        }) => operation(config, books, revision),
        _ => Err(failure(
            "Library",
            method,
            "expects a std.lang.Library value",
...
fn with_runtime<T>(
    handle: u64,
    method: &str,
    operation: impl FnOnce(&Value, &bool, &i64) -> Result<T, String>,
) -> Result<T, String> {
    STATE.with(|state| match state.borrow().records.get(&handle) {
        Some(Record::Runtime {
            config,
            closed,
            revision,
        }) => operation(config, closed, revision),
        _ => Err(failure(
            "Runtime",
            method,
            "expects a std.lang.Runtime value",
...
fn with_harness<T>(
    handle: u64,
    method: &str,
    operation: impl FnOnce(&Value, &u64, &u64, &bool) -> Result<T, String>,
) -> Result<T, String> {
    STATE.with(|state| match state.borrow().records.get(&handle) {
        Some(Record::Harness {
            config,
            ...
            runtime,
            closed,
        }) => operation(config, library, runtime, closed),
        _ => Err(failure(
            "Harness",
            method,
            "expects a std.lang.Harness value",
...
fn snapshot_library(
    ...
    STATE.with(|state| match state.borrow().records.get(&snapshot) {
        Some(Record::Snapshot {
            ...
            library_revision,
            ..
        }) if *snapshot_owner == owner => Ok((books.clone(), *library_revision)),
        _ => Err(failure(
            "Library",
            method,
            "requires a snapshot from the same Library",
        )),
    })
}
...
fn snapshot_harness(
    ...
    owner: u64,
    method: &str,
) -> Result<(HashMap<String, Value>, i64, bool, i64, bool), String> {
    STATE.with(|state| match state.borrow().records.get(&snapshot) {
        Some(Record::Snapshot {
            kind: SnapshotKind::Harness,
            owner: snapshot_owner,
        ...
        _ => Err(failure(
            "Harness",
            method,
            "requires a snapshot from the same Harness",
        )),
    })
}
...
fn optional_handle(
    ...
    lookup(config, name)
        .map(|value| handle(&value, expected, method))
        .transpose()
}
...
fn map(entries: Vec<(&str, Value)>) -> Value {
    Value::Map(PMap::from_iter(
        entries
            .into_iter()
            .map(|(key, value)| (Value::Keyword(Keyword::from(key)), value)),
    ))
}
